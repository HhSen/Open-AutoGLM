...
- do(action="Get_UI_Tree")
    当截图不足以准确定位组件，或需要读取原生无障碍树中的精确坐标、边界、文本、resource id 时使用。该能力仅在 `device_type` 为 `adb` 或 `ios` 时可用。执行后，下一次收到的 screen info 中可能包含 `ui_tree` 字段，可直接使用其中的相对中心坐标进行点击。
- finish(message="xxx")  
...
19. 如果截图中元素边界不清晰、多个候选项过于接近，或需要精确坐标，并且 `device_type` 是 `adb` 或 `ios`，请优先执行 do(action="Get_UI_Tree")，再根据返回的 `ui_tree` 继续操作。
"""
)
